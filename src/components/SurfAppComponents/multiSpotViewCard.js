...
const BackDrop = styled.div`
  ...
  position: relative;
  height: ${(props) =>
    props.dynamicHeight > 8 ? props.dynamicHeight / 6 + 22 : 22}vh;
  width: 100%;
  padding: 0.5vh;
`;
const Cell = styled(BackDrop)`
  ...
  @media (max-width: ${variables.medium}) {
    height: calc((30vw - 1vw) * 0.65);
    max-height: calc((350px * 0.33) - 1.5vw);
  }
`;
...
const ConditionContainer = styled.div`
  ...
  padding: 0 5px;
  background: ${(props) =>
    getRatingColor(props.currentMultiViewConditions)};
  z-index: 1;
  box-shadow: 0 2.8px 2.2px rgb(0 0 0 / 3%),
    0 6.7px 5.3px rgb(0 0 0 / 5%), 0 12px 8px rgb(0 0 0 / 3%),
    0 12px 8px rgb(0 0 0 / 4%), 0 12px 8px rgb(0 0 0 / 3%),
    0 12px 8px rgb(0 0 0 / 3%);
  @media (max-width: ${variables.large}) {
    position: absolute;
    min-width: calc(30% - 1.5vh);
    max-width: fit-content;
...
const mapDispatchToProps = (dispatch) => ({
  getLocationsObject: (locations) =>
    dispatch(getLocationsObject(locations)),
  getCloseSurfSpots: (closeSurfSpots) =>
    dispatch(getCloseSurfSpots(closeSurfSpots)),
  getSurfForecast: (surfForecast) =>
    dispatch(getSurfForecast(surfForecast)),
  getSwellForecast: (swellForecast) =>
    dispatch(getSwellForecast(swellForecast)),
  getWindForecast: (windForecast) =>
    dispatch(getWindForecast(windForecast)),
  getMaxWaveHeight: (maxWaveHeight) =>
    dispatch(getMaxWaveHeight(maxWaveHeight)),
  getTideStations: (tideStations) =>
    dispatch(getTideStations(tideStations)),
  getTideForecast: (tideForecast) =>
    dispatch(getTideForecast(tideForecast)),
  getNdbcStations: (ndbcStations) =>
    dispatch(getNdbcStations(ndbcStations)),
  getWaterTemp: (waterTemp) => dispatch(getWaterTemp(waterTemp)),
  getCurrentSwell: (currentSwell) =>
    dispatch(getCurrentSwell(currentSwell)),
  getWeatherStations: (weatherStations) =>
    dispatch(getWeatherStations(weatherStations)),
  getWeather: (weather) => dispatch(getWeather(weather)),
  getWeatherForecast: (weatherForecast) =>
    dispatch(getWeatherForecast(weatherForecast)),
  searchOpenState: (openState) =>
    dispatch(searchOpenState(openState)),
  closeSpotsOpenState: (openState) =>
    dispatch(closeSpotsOpenState(openState)),
  loadView: (openState) => dispatch(loadView(openState)),
  getActiveLocation: (activeLocation) =>
    dispatch(getActiveLocation(activeLocation)),
  getMultiViewForecast: (forecast) =>
    dispatch(getMultiViewForecast(forecast)),
  getMultiViewSwellForecast: (forecast) =>
    dispatch(getMultiViewSwellForecast(forecast)),
  getMaxWaveHeightMultiView: (waveHeight) =>
    dispatch(getMaxWaveHeightMultiView(waveHeight)),
  getActiveSurfSpot: (surfSpot) =>
    dispatch(getActiveSurfSpot(surfSpot)),
...
const getRatingText = (currentMultiViewConditions) => {
  if (
    currentMultiViewConditions.minBreakingHeight >= 5 &&
    currentMultiViewConditions.fadedRating < 1
  ) {
    return 'Very Good';
  } else if (
    currentMultiViewConditions.minBreakingHeight >= 3 &&
    currentMultiViewConditions.maxBreakingHeight >= 5 &&
    currentMultiViewConditions.fadedRating < 1
  ) {
    return 'Good';
  } else if (
    currentMultiViewConditions.minBreakingHeight >= 2 &&
    currentMultiViewConditions.maxBreakingHeight === 4 &&
    currentMultiViewConditions.fadedRating <= 1
  ) {
    return 'Fair - Good';
  } else if (
    currentMultiViewConditions.minBreakingHeight >= 2 &&
    currentMultiViewConditions.maxBreakingHeight >= 3 &&
    currentMultiViewConditions.fadedRating <= 2
  ) {
    return 'Fair';
  } else if (
    currentMultiViewConditions.minBreakingHeight >= 1 &&
    currentMultiViewConditions.maxBreakingHeight <= 3 &&
    currentMultiViewConditions.fadedRating <= 2
  ) {
    return 'Poor - Fair';
  } else if (
    (currentMultiViewConditions.minBreakingHeight >= 0 &&
      currentMultiViewConditions.maxBreakingHeight <= 2) ||
    currentMultiViewConditions.fadedRating >= 3
  ) {
    return 'Poor';
...
class MultiSpotViewCard extends Component {
  ...
  render() {
    return this.props.multiViewSwellForecast.map((spot, i) => {
      ...
      const getFutureConditions = (data) => {
        ...
        const filteredArr = data.filter((d) => {
          const forecastDateObj = new Date(d.time).getTime();
          const fullDateToday = Math.floor(Date.now() / 1000) * 1000;
          const parsedDate = new Date(fullDateToday);
          const endTime = addHours(8, parsedDate);
          const formattedHours = formatAMPM(new Date(d.time));
          d.formattedHours = formattedHours;
          return (
            forecastDateObj >= fullDateToday &&
            forecastDateObj <= endTime
          );
        });
      ...
      let currentMultiViewConditions = getCurrentConditions(
        spot.forecast
      )[getCurrentConditions(spot.forecast).length - 1];

      const rating = currentMultiViewConditions
        ? [
            currentMultiViewConditions.solidRating,
            currentMultiViewConditions.fadedRating,
          ]
        : null;
      const finalDeg = currentMultiViewConditions.windDirection - 180;
      const windSpeed = parseInt(
        currentMultiViewConditions.windSpeed
      );
      ...
      return (
        <CurrentConditionBackdrop
          key={i}
          onClick={() => {
            this.props.getActiveLocation(spot);
            this.props.getSurfForecast({
              surfSpot: spot.spotId,
              apiEndpoints: this.props.surf.surfApiEndPoints,
            });
            this.props.getWeather(spot);
            this.props.getWeatherForecast(spot);
            ...
            <TitleCol xs={12}>
              <Row>
                <MultiViewCardColumn xs={5}>
                  <Location>
                    {spot.town}, {spot.countryOrState}
                  </Location>
                  <Distance>{`${convertMilesToKM(
                    spot.distanceFromLocation
                  )} miles away`}</Distance>
                  <WaveHeightWrapper>
                    <p>
                ...
                <MultiViewCardColumn xs={7}>
                  <Cell>
                    <CellCol xs={7}>
                      ...
                      <PrimarySwell>
                        ...
                        {`${Math.round(
                          MetersToFeet(
                            currentMultiViewConditions.waveHeight
                          )
                        )}ft at ${Math.round(
                          currentMultiViewConditions.wavePeriod
                        )} sec from the ${DegreesToCompassDirection(
                          currentMultiViewConditions.waveDirection
                        )}`}
                      </PrimarySwell>
                      {currentMultiViewConditions.swellHeight ? (
                        <SecondarySwell>
                          <SwellKeyColor>
                            <rect width="100%" height="100%" />
                          </SwellKeyColor>
                          {`${Math.round(
                            ...
                            currentMultiViewConditions.swellPeriod
                          )} sec from the ${DegreesToCompassDirection(
                            currentMultiViewConditions.swellDirection
                          )}`}
                        </SecondarySwell>
                      ) : null}
                      <SwellKey>
                  ...
                  <Cell>
                    <CellCol xs={7}>
                      ...
                      <WindData>
                        {currentMultiViewConditions.windSpeed} mph
                        <span> with gusts of</span> <br></br>
                        {currentMultiViewConditions.windGusts} mph
                        <span>out of the </span>
                        {DegreesToCompassDirection(
                          currentMultiViewConditions.windDirection
                        )}
                      </WindData>
                    </CellCol>
                    <CellCol xs={5}>
                      <CurrDataComponentMultiContainer>
                        <CurrWindDataComponentMulti
                          msWindForecast={currentMultiViewConditions}
                          weatherForecast={spot.currentWeather}
                        />
                      </CurrDataComponentMultiContainer>
                    </CellCol>
                  </Cell>
            ...
            <AllChartsContainer>
              <ChartCol xs={6}>
                <SwellBarChartMultiView
                  ...
                  forecast={getFutureConditions(spot.forecast)}
                />
              </ChartCol>
              <ChartCol xs={6}>

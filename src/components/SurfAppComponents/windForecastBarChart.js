  ...
  return (
    <image
      style={{
        ...
        opacity:
          props.index === props.data.focusBar
            ? '1'
            : props.data.mouseLeave
            ? '0.7'
            : '0.4',
        position: 'relative',
        zIndex: '2',
...
const WindInfoTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <WindChartToolTip>
        <WindChartDateTime>
          {payload[0].payload.dayOfWeek}, {payload[0].payload.date} -{' '}
        ...
        <WindChartWaveHeight>
          ...
          <span>
            {' '}
            {payload[0].payload.speed} - {payload[0].payload.gusts}mph
          </span>
        </WindChartWaveHeight>
        <WindChartWaveHeight>
          Wind Direction:
    ...
    return (
      <Fragment>
        <MediaQuery minWidth={Number(variables.largeNum)}>
          <ResponsiveContainer
            className="windForecastContainer"
            width="100%"
            height="90%"
          >
            <BarChart
            ...
            >
              ...
              <Bar
                dataKey="speed"
                shape={<WindBarb data={this.state} />}
              ></Bar>
            </BarChart>
          </ResponsiveContainer>
        </MediaQuery>
